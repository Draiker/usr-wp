...
class Stats {
	...
	public function set_filters() {
		...
		if ( isset( $_GET['start'] ) ) {
			...
			$this->start_date = strtotime( '4 month ago', $this->current_time );
		}
	...
	public function dashboard_widget_callback(){
		...
		if( ! $total_entries ) {
			...
			$html .= "<p class='ssps_last_30_days_graph_text'>".__('Last 120 Days', 'seriously-simple-stats')." <a href='".admin_url("edit.php?post_type=podcast&page=podcast_stats")."'>".__('View All Data', 'seriously-simple-stats')."</a>"."</p>";

			$html .= "<div id='weekly_listens'></div>";

			$this->start_date = strtotime( current_time( 'Y-m-d H:i:s' ) . ' -120 DAY' );
